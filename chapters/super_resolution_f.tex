...
なお、本章で述べる手法および検証結果の一部については、
国際会議CIF 2023、CLEO-PR 2024にて報告を行っている\cite{cif_2023}\cite{cleopr_2024}。
...
\subsection{CNN}
CNNは全結合層とは異なり、入力データの空間的な局所性を保持したまま情報を処理できる。

CNNにおける主要な演算である畳み込みは、入力画像 $X$ に対して、特定の重みを持つフィルタ（カーネル） $W$ をスライドさせながら積和演算を行う。
...
画像からエッジ、テクスチャ、あるいはより抽象的な物理的特徴を抽出することが可能となる。

\subsection{SRCNN}
SRCNNは、単一画像超解像のためのエンドツーエンドな学習モデルである。
...
20 nmメッシュでの計算と比較して、40 nmメッシュでは計算時間を短縮できるため、
...
PSNRの改善幅は約1 dBに留まった図(\ref{fig:general100_result})。
...
\begin{figure}[htbp]
  ...
  \caption{一般画像を学習したモデルを用いてシミュレーション画像を超解像した結果。}
  \label{fig:general100_result}
\end{figure}

\subsection{Cバンド学習モデルの評価}
Cバンドのシミュレーション画像を学習データとして用いたモデルを評価した。
結果として、PSNRは大幅に向上し、約 8 dBの改善が見られた(図\ref{fig:C-band})。
...
結果として、Cバンドでの推論時と同等の約30 dbのPSNR値が得られた(図\ref{fig:O-band})。
