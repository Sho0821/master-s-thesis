...
再構成精度の定量的な評価として、PSNRおよびLPIPSを算出した。結果を図\ref{fig:psnr_e_i}、図\ref{fig:lpips_e_i}に示す。

Cバンドでの推論結果は30 dBを超える高い値を示し、
Oバンドでの推論結果においても約30 dBという値を達成した。

一方で、LPIPSの数値は約0.3弱を記録した。
この要因として、強度分布画像においてピークが
集中する領域でのわずかなコントラストの変化等が、
LPIPSが重視する知覚的な特徴に敏感に反映されたためと考えられる。

しかし、視覚的な比較結果からも明らかな通り、
干渉パターンの全体像やピークの位置、
導波路内での閉じ込めといった設計資産として再利用する上で
必要な物理的特徴は捉えられている。
PSNRに関しては良好であることから、この結果は、ある波長帯域で構築した変換モデルが、
設計変更によって波長が変わったとしても、
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/lpips_e_i.png}
  \caption{再構成された強度分布画像のLPIPS評価結果。}
  \label{fig:lpips_e_i}
\end{figure}
...
図\ref{fig:pix2pix_graph}、図\ref{fig:pix2pix_graph2}、図\ref{fig:pix2pix_graph_lpips}、図\ref{fig:pix2pix_graph2_lpips}に示す。
検証の結果、注目すべき点として、強度画像を入力に含まない場合であっても、
PSNRが30 dBに達するケースが確認され、LPIPSも0.1を大きく下回る結果となった。
...
一方で、強度分布画像を追加することでPSNRはさらに向上し、LPIPSもさらに低下している。
...
\begin{figure}[htbp]
  ...
  \caption{テストデータ(波長1550nm,1551nm,1552nm)に対するLPIPS評価結果(入力に強度分布画像無し)。}
